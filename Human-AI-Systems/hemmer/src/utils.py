...
def concat_args(args, mode):
    ...
    if mode == 'true_expert' or mode == 'full_automation':
        try:
...
def load_from_checkpoint(train_dir, classifier, allocation_system, optimizer, scheduler, seed):
    ...
    try:
        ...
        if allocation_system is not None:
            allocation_system.load_state_dict(checkpoint['allocation_sys_state_dict'])
    ...
    return classifier, allocation_system, optimizer, scheduler, epoch, test_metrics


def save_to_checkpoint(train_dir, epoch, classifier, allocation_system, optimizer, scheduler, test_metrics, seed):
    """Save to checkpoint
    ...
    if allocation_system is not None:
        torch.save({'epoch': epoch,
                    ...
                    'allocation_sys_state_dict': allocation_system.state_dict(),
...
def log_test_metrics(writer, epoch, test_metrics, num_classes):
    ...
    :return:
    """
    writer.add_scalar('system/accuracy/test', test_metrics['System Accuracy'], epoch)
    writer.add_scalar('system/loss/test', test_metrics['System Loss'], epoch)
    writer.add_scalar('classifier/accuracy', test_metrics['Classifier Accuracy'], epoch)
    writer.add_scalar('classifier/task_subset_accuracy', test_metrics['Classifier Task Subset Accuracy'], epoch)
    writer.add_scalar('classifier/coverage/total', test_metrics['Classifier Coverage'][0], epoch)
    writer.add_scalar('expert/accuracy', test_metrics['Expert 1 Accuracy'], epoch)
    writer.add_scalar('expert/task_subset_accuracy', test_metrics['Expert 1 Task Subset Accuracy'], epoch)
    writer.add_scalar('expert/coverage/total', test_metrics['Expert 1 Coverage'][0], epoch)
    for c in range(num_classes):
        writer.add_scalar('classifier/coverage/class' + str(c), test_metrics['Classifier Coverage'][1][c], epoch)
