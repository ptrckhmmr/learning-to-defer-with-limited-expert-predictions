...
from src.train import run_team_performance_optimization, run_full_automation
...
def main(argv):
    ...
    NUM_HIDDEN_UNITS = 100 if FLAGS.dataset == 'cifar100' else 30
    ...
    args = {'approach': APPROACH, 'ex_strength': EX_STRENGTH, 'num_experts': NUM_EXPERTS,
            'num_hidden_units': NUM_HIDDEN_UNITS, 'binary': BINARY}
    ...
    if APPROACH == 'TrueExpert':
        ...
        print('Get performance of collaboration with the true expert:')
        true_ex_accuracy, true_ex_coverage = \
            run_team_performance_optimization("true_expert", args, 123, true_expert_fns, EPOCHS, NUM_HIDDEN_UNITS,
                                              train_batch_size=TRAIN_BATCH_SIZE,
                                              test_batch_size=TEST_BATCH_SIZE,
                                              binary=BINARY,
                                              num_classes=NUM_CLASSES)
        accuracies.append(true_ex_accuracy)
        coverages['cov'].append(true_ex_coverage[0])
        coverages['cov_by_class'].append(true_ex_coverage[1])
    # run the full automation baseline
    elif APPROACH == 'FullAutomation':
        accuracies = []
        coverages = None
        full_automation_accuracy = run_full_automation(args, 123, EPOCHS, NUM_HIDDEN_UNITS,
                                                       train_batch_size=TRAIN_BATCH_SIZE,
                                                       test_batch_size=TEST_BATCH_SIZE)
        accuracies.append(full_automation_accuracy)
        print("-" * 40)
    else:
        ...
        for seed in SEEDS:
            ...
            for i, p in enumerate(predictions):
                try:
                    ...
                    args['approach'] = p
                    pred_ex_accuracy, pred_ex_coverage = \
                        run_team_performance_optimization("pred_expert", args, seed, pred_expert_fns[p], EPOCHS,
                                                          NUM_HIDDEN_UNITS,
                                                          train_batch_size=TRAIN_BATCH_SIZE,
                                                          test_batch_size=TEST_BATCH_SIZE,
                                                          binary=BINARY,
                                                          num_classes=NUM_CLASSES)
                    accuracies[keys[i]].append(pred_ex_accuracy)
                    coverages[keys[i]]['cov'].append(pred_ex_coverage[0])
                    coverages[keys[i]]['cov_by_class'].append(pred_ex_coverage[1])
...
if __name__ == '__main__':
    flags.DEFINE_string('approach', 'EmbeddingNN_bin', 'Approach for predicting the expert labels')
